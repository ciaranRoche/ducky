...
1. **Get current sprint overview:**
   ```bash
   jira sprint list --current -p ${DUCKY_JIRA_PROJECT:-HYPERFLEET} --plain 2>/dev/null || echo "Error: Could not fetch sprint. Is jira-cli configured?"
   ```

2. **Get user's assigned tickets in current sprint:**
   ```bash
   jira sprint list --current -p ${DUCKY_JIRA_PROJECT:-HYPERFLEET} -a$(jira me) --plain 2>/dev/null
   ```

3. **Get ticket status breakdown:**
   ```bash
   jira issue list -q"project = ${DUCKY_JIRA_PROJECT:-HYPERFLEET} AND assignee = currentUser() AND sprint in openSprints()" --plain 2>/dev/null
   ```
