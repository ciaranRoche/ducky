...
.mcp.json        MCP server configuration (MCPVault for Obsidian, Qdrant for writing style RAG)
```
...
- **MCP:** The `obsidian` MCP server connects to the local vault via MCPVault. For cross-project access, run `/vault-setup` to configure at user scope.
